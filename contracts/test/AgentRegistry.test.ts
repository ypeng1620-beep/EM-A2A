...
describe('AgentRegistry', () => {
  ...
  describe('Registration', () => {
    ...
    it('should emit AgentRegistered event', async () => {
      ...
      await expect(registry.connect(agent1).registerAgent(did, metadata))
        .to.emit(registry, 'AgentRegistered')
        .withArgs(did, agent1.address)
    })

    it('should reject empty DID', async () => {
      await expect(registry.connect(agent1).registerAgent('', '{}')).to.be.revertedWith(
        'DID required',
      )
    })
    ...
    it('should reject duplicate DID', async () => {
      ...
      await expect(registry.connect(agent2).registerAgent(did, '{}')).to.be.revertedWith(
        'DID exists',
      )
    })
    ...
    it('should reject address registering twice', async () => {
      ...
      await expect(registry.connect(agent1).registerAgent(did2, '{}')).to.be.revertedWith(
        'Address has DID',
      )
    })
  ...
  describe('Metadata', () => {
    ...
    it('should reject unauthorized metadata update', async () => {
      ...
      await expect(registry.connect(agent2).updateMetadata(did, '{}')).to.be.revertedWith(
        'Not authorized',
      )
    })
  })
  ...
  describe('Status', () => {
    ...
    it('should reject non-owner status change', async () => {
      ...
      await expect(registry.connect(agent1).setAgentStatus(did, false)).to.be.reverted
    })
  })
  ...
  describe('Credentials', () => {
    it('should issue a credential', async () => {
      ...
      const [holderDID, cType, cMeta, issuedAt, cExpiresAt, active] =
        await registry.getCredential(credId)
    ...
    it('should revoke a credential', async () => {
      ...
      await registry
        .connect(owner)
        .issueCredential(credId, did, 'code_audit', '{}', Date.now() + 365 * 86400)

      await expect(registry.connect(owner).revokeCredential(credId))
    ...
    it('should reject credential for inactive agent', async () => {
      ...
      await expect(
        registry
          .connect(owner)
          .issueCredential('cred_x', did, 'code_audit', '{}', Date.now() + 365 * 86400),
