...
describe("EscrowService", () => {
  ...
  describe("Order Creation", () => {
    it("should create a funded order", async () => {
      ...
      ).to.emit(escrow, "OrderCreated").withArgs(1, buyer.address, seller.address, amount)
    ...
    it("should reject past deadline", async () => {
      const deadline = Math.floor(Date.now() / 1000) - 3600
      await expect(
        escrow.connect(buyer).createOrder(seller.address, deadline, "{}", { value: ethers.parseEther("1") })
  ...
  describe("Delivery Flow", () => {
    beforeEach(async () => {
      const deadline = Math.floor(Date.now() / 1000) + 3600
      await escrow.connect(buyer).createOrder(seller.address, deadline, "{}", { value: ethers.parseEther("1") })
    })

    it("should allow seller to confirm delivery", async () => {
      await expect(
        escrow.connect(seller).confirmDelivery(1)
      ).to.emit(escrow, "OrderDelivered").withArgs(1)
    ...
    it("should allow buyer to accept delivery and release funds", async () => {
      ...
      await expect(
        escrow.connect(buyer).acceptDelivery(1)
      ).to.emit(escrow, "OrderAccepted").withArgs(1)
    ...
    it("should reject non-buyer acceptance", async () => {
      await escrow.connect(seller).confirmDelivery(1)
      await expect(
        escrow.connect(seller).acceptDelivery(1)
  ...
  describe("Disputes", () => {
    ...
    it("should allow buyer to dispute", async () => {
      await expect(
        escrow.connect(buyer).dispute(1, "Not as described")
      ).to.emit(escrow, "OrderDisputed").withArgs(1, "Not as described")
    ...
    it("should allow owner to resolve dispute with refund", async () => {
      ...
      ).to.emit(escrow, "OrderResolved").withArgs(1, 6) // RESOLVED_REFUNDED
    })
    ...
    it("should allow owner to resolve dispute with release", async () => {
      ...
      ).to.emit(escrow, "OrderResolved").withArgs(1, 5) // RESOLVED_RELEASED
    })
  })
