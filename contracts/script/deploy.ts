...
async function main() {
  ...
  console.log(`Balance: ${ethers.formatEther(await ethers.provider.getBalance(deployer.address))} TRX\n`)
  ...
  for (const p of productIds) {
    ...
    try {
      ...
      if (p.mode === 1) {
        ...
        const tx = await revenueSplitter.registerProduct(pid, p.mode, p.rate, tiers, p.rate * 20 / 100, p.rate * 5)
        await tx.wait()
      } else {
        const tx = await revenueSplitter.registerProduct(pid, p.mode, p.rate, [], p.rate * 20 / 100, p.rate * 5)
        await tx.wait()
      }
